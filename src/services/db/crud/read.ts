...
): Promise<QueryResult<T["findFirstOrThrow"], E>> =>
  base(model, () => (model.findFirstOrThrow as Function)({ where: filter, ...omit((options || {}) as any, ["error"]) }), options);

export const getMany = <T extends Delegate>(
  model: T,
  filter: QueryFilter<T["findMany"]>,
  options?: QueryOptions<T["findMany"]>
): Promise<QueryResult<T["findMany"]>> =>
  base(model, () => (model.findMany as Function)({ where: filter, ...omit((options || {}) as any, ["error"]) }), options);
...
): Promise<QueryResult<T["findUniqueOrThrow"]>> =>
  base(model, () => (model.findUniqueOrThrow as Function)({ where: filter, ...omit((options || {}) as any, ["error"]) }), options);

export const getById = async <T extends Delegate>(
  model: T,
  id: string,
  options?: QueryOptions<T["findFirstOrThrow"]>
): Promise<QueryResult<T["findFirstOrThrow"]>> =>
  base(model, () => (model.findFirstOrThrow as Function)({ where: { id }, ...omit((options || {}) as any, ["error"]) }), options);
